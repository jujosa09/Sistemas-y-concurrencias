...
TParking inicializar(){
    TParking park = malloc(sizeof (struct Plaza));
    if(park == NULL){
        perror("Error al inicializar parking");
    }
}
...
void mostrarPlaza(const TPlaza *pl){
    printf("Tipo: %e   Matricula: %c",(*pl) .tipo, (*pl) . matricula);
} //NOTA: TPlaza * es equivalente a TParking
